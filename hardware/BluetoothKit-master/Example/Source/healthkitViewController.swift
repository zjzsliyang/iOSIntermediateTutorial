...
//  BKExample
//
//  Created by yuannnn on 2017/8/16.
//  Copyright © 2017年 Rasmus Taulborg Hummelmose. All rights reserved.
//
import Foundation
import UIKit
import HealthKit
class healthkitViewController: UIViewController {
    ...
    @IBAction func startAct(_ sender: Any) {
        alarmTime = Date()
        if (!timer.isValid) {
            let aSelector : Selector = #selector(healthkitViewController.updateTime)
    ...
    @IBAction func activateHealthKit(_ sender: Any) {
        ...
        if #available(iOS 10.0, *) {
            ...
            self.healthStore.requestAuthorization(toShare: typestoRead, read: typestoRead) { (success, error) -> Void in
                if success == false {
